...
- **Prefix/folder-style object browsing**: Drills into a bucket like a file manager — folders are S3 "common prefixes," not real objects, so navigating never lists more of the bucket than the current level. A clickable breadcrumb (an "s3" root back to the bucket list, then the bucket, then every path segment) jumps back to any ancestor in one click; paginated via Load More for buckets with more than 1000 keys at a level
...
- **Upload**: Pick a local file and upload it into the currently browsed bucket/prefix, with the destination key defaulted to the current prefix + filename (editable). Since S3 has no versioning by default, uploading to an existing key would silently overwrite it — so before ever calling `PutObject`, the destination is checked with `HeadObject` and an explicit overwrite confirmation is required if something's already there. A simple indeterminate progress indicator shows during the upload, not a byte-level progress bar. Dragging a single local file onto the object table is a faster shortcut to the same dialog, pre-selected — it's not a bypass; the Upload button and overwrite check still apply exactly as if you'd picked the file yourself. Dropping more than one file at once is rejected with a message rather than silently uploading just one of them
- **Upload Multiple**: Pick any mix of local files and/or folders in one dialog — a folder is walked recursively, preserving its own name and relative structure as key prefixes. Shows the total file count/size up front for confirmation before anything else happens (a safety net against an unexpectedly huge folder), then checks every destination and shows a *single* aggregate choice if any already exist (Overwrite All / Skip Existing / Cancel) rather than one dialog per file. Progress is a real determinate "N of M" indicator, not an indeterminate spinner. A failed file is skipped, not retried, so the rest of the batch still completes; the end-of-batch summary lists exactly which keys failed and why, not just a count
...
- **Download Selected**: Multi-select any mix of objects and folders in the object browser and download them all at once — a selected folder is recursively expanded (every object under that prefix, not just one level) and its own name/structure is recreated as local subdirectories under wherever you choose to save. Same batch shape as Upload Multiple: total object count/size confirmed up front, a single aggregate choice if local files already exist (Overwrite All / Skip Existing / Cancel) instead of one dialog per file, a real determinate progress indicator, and a failed object is skipped (not retried) with the end-of-batch summary listing exactly which keys failed and why
...
This is primarily a **browse** tool, mirroring dynamodb-client's and lambda-inspector's "browse, don't mutate destructively" ethos. The one mutating AWS action is uploading a file (see Features above), which always confirms before overwriting an existing object — batch upload/download both extend this to many files/a folder at once, but keep the same confirm-before-overwrite ethos at the aggregate level rather than dropping it for convenience. Downloading is a read-only `GetObject` call, so it isn't mutating AWS state, but still confirms before overwriting a local file for the same reason. No delete, no bucket mutation, no ACL/policy changes. If scope grows beyond that, it should get the same explicit confirmation-dialog treatment the upload-overwrite check already has, not be added silently. Open in Console is a deliberate exception to "browse, don't mutate" in one sense: it hands the user a real, fully-authenticated AWS Console session, which can do anything the connected role's IAM permissions allow — including delete, bucket mutation, and everything else this app itself refuses to do. s3-browser's own UI still never issues a mutating call beyond upload (single or batch); what the user does after Open in Console is on the console (and the role's permissions), not this app.
